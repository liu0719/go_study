...
所以"用 Cookie 做登录"的完整内容是：Cookie 里放一个**凭据**。这个凭据要么自己能被验证（无状态，JWT），要么服务端拿它去查表（有状态，Session）。本节三种方案就是这三种玩法。
...
**解决方法**  1.session复制，每台服务器都配置一份相同的sessionId，浪费资源
